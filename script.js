...
function initMap() {
    ...
    google.maps.event.addListener(
        map,
        "bounds_changed",() => {
            fetch(`http://api.openweathermap.org/data/2.5/box/city?appid=dca79b4ccfd8225e027e893888f05199&bbox=${map.getMapBbox()}`)
            .then(function(response) {
                response.json().then(function(data) {
                  if (data.list) {
                      if (!downloadedCities.includes(data.list[0].name)) {
                        fetch(`http://api.openweathermap.org/data/2.5/weather?q=${data.list[0].name}&appid=dca79b4ccfd8225e027e893888f05199`)
                        .then(function(res) {
